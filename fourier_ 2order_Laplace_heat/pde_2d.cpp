...
void make_c_from_d_and_previous(double *matrix_d, double *matrix_c, double *previous_nodes, int Nx, int Ny, double tau)
{
    for(int n = 1; n < Ny; n++)
    {
        for(int m = 1; m < Nx; m++)
        {
            //matrix_c[(n) * (Nx) + m] = (matrix_d[(n) * (Nx) + m] + (previous_nodes[(n) * (Nx) + m] / tau)) / (lambda_m(m, Nx) + lambda_m(n, Ny) + (1. / tau));
            matrix_c[(n) * (Nx) + m] = (matrix_d[(n) * (Nx) + m] * tau + (previous_nodes[(n) * (Nx) + m])) / ((lambda_m(m, Nx) + lambda_m(n, Ny))*tau + 1.);
        }
    }
}
