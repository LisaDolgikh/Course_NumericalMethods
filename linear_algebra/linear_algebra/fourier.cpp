...
void make_b_from_x(double *A, double *x, int N, double *b)
{
    for (int k = 0; k < N; k++)
    {
        b[k] = 0.;
...
/*
void make_fourier_matrix_without_0(double *A, int N, double p) // размер N * N
{
    double h_reverse = (double)(N + 1)- 0.5;  // h^(-1)
    double h_reverse_square = h_reverse * h_reverse;
    ...
    for(int i = 0; i < N; i++)
    {
        ...
        {
            if (i == j)
                A[i * N + j] = p + 2.0 * h_reverse_square;
            else if (i - j == 1 || i - j == -1)
                A[i * N + j] = -1.0 * h_reverse_square;
            else
                A[i * (N + 1) + j] = 0.;
        }
    }
    A[(N - 1) * N + N - 1] = h_reverse_square + p ;
}*/
